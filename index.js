...
let translateZ = ({x, y, z}, dz) => {
  return {x, y, z: z + dz};
}

let rotate = ({x, y, z}, theta) => {
...
/*   for (let v of vertices) {
    point(screen(projection(translateZ(rotate(v, theta), dz))));
  } */

  for (let f of faces) {
    for (let i = 0; i < f.length; i++) {
      let a = vertices[f[i]];
      let b = vertices[f[(i + 1) % f.length]];
      lines(
        screen(projection(translateZ(rotate(a, theta), dz))),
        screen(projection(translateZ(rotate(b, theta), dz)))
      )
    }
  }

  setTimeout(framing, 1000/fps)
}
